# Salesforce AI Engineer Journey

Documenting my progression into AI — leveraging 6 years of 
Salesforce architecture experience as a technical moat rather than 
background to leave behind.


## Projects

| # | Project | Stack | Status |
|---|---|---|---|
| 1 | [Salesforce Well-Architected RAG Pipeline](./projects/project-1-salesforce-rag) | Python · ChromaDB · OpenAI · LangSmith · DeepEval | ✅ Complete |
| 2 | [Salesforce Architect Agent](./projects/project-2-salesforce-data-architect-agent) | MCP | ✅ Complete |
| 3 | CRM Knowledge Worker + Text-to-SOQL | Agentic RAG · FastAPI | 📅 Planned |
| 4 | Multi-Agent SDR Crew | CrewAI · MCP · Salesforce API | 📅 Planned |
| 5 | Fine-Tuned Salesforce Expert LLM | Unsloth · QLoRA · DeepEval | 📅 Planned |
| 6 | Salesforce AI Architect Suite | FastAPI · React · Langfuse | 📅 Planned |

## Background

- **Current:** Salesforce Application Architect, Accenture
- **Certifications:** Application Architect · Data Architect · 
  Platform Developer I · Agentforce Specialist
- **AI Stack:** Python · LangGraph · CrewAI · MCP · ChromaDB · 
  Pinecone · Unsloth · LangSmith · FastAPI

## Connect

[LinkedIn](https://www.linkedin.com/in/devwright/) · 
[Hugging Face](https://huggingface.co/DevonWright)
